...
3. As a retrieval engineer, I want to sweep N = 20/50/100 for the candidate model (3 combinations + baseline) and have per-combination metrics (nDCG@5, MRR, answer quality, p50/p95 latency, memory, cold/warm load) reported in one pass.
...
6. As a retrieval engineer, I want a `Reranker` seam whose contract is an ordering contract — absolute scores carry no meaning, adapters need not share a score scale, the only guarantee is deterministic ordering with ChunkID ASC tie-break — so that model adapters are interchangeable.
...
- **`Reranker` seam (ADR-0044).** A new deep-module seam abstracting model behavior only: `Rerank(ctx, query, candidates) ([]ScoredCandidate, error)`-shaped surface. Adapter: cross-encoder (BGE-reranker-v2-m3), probe-side. The seam owns the ordering contract: no absolute-score interpretation, adapters need not share a score scale, deterministic ordering with ChunkID ASC tie-break is the only guarantee.
...
- **Probe tooling (ADR-0045).** Extends the existing eval harness (ADR-0027): baseline run with candidate artifact recording; per-combination simulation runs (1 model x 3 N) over the recorded artifacts; MPI/MAR gate evaluation; benchmark manifest (fingerprint, gold set version, operational budgets frozen before the probe). Candidate artifact format is normative (ADR-0045): fingerprint, query id, ordered candidate ids, baseline retrieval metadata, candidate scores informational-only, reranker output ordering.
...
- Model pool expansion beyond the ADR-0043 candidate.
