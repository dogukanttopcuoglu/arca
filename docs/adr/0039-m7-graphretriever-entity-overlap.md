...
- **Output contract:** `NewGraphRetriever(store GraphStore, contentStore ContentStore)`. `SearchResult` carries `ChunkID`, `Score`, `ContentMarkdown` resolved from the `ContentStore` (the same access model as the dense retriever — no second content source), and minimal `Metadata{ChunkID}`. Section/document metadata is not populated in v1 (accepted trade-off: citations may show unknown sections); the ADR-0038 node schema stays unchanged. Results are truncated to `query.TopK`.
