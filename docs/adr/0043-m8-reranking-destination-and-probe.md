...
- **Approach selection by probe, not assumption.** An offline probe measures two architecture families against each other before any implementation decision: cross-encoder (candidate: BGE-reranker-v2-m3) and late interaction (candidate: ColBERTv2). Each family is represented by a single mature candidate — the probe selects the approach, it does not validate a model.
